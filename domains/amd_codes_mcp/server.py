"""amd-codes-mcp MCP stdio server.

Boot:

  1. Settings.from_env(domain="codes").
  ...
  5. Load per-action policies from knowledge/integrations/amd/codes/.
...
`WRITE_TOOLS_ENABLED = False` is the structural safety belt. Codes is
a pure read-only reference-data domain so this constant is trivially
safe (no write handlers exist) — but it ships explicitly for parity
with sibling per-domain servers.
"""
...
_LOG = logging.getLogger("amd_codes_mcp.server")


# ---- Structural safety belt -----------------------------------------------
WRITE_TOOLS_ENABLED: bool = False
DOMAIN: str = "codes"
SERVER_NAME: str = "amd-codes-mcp"
EXPECTED_TOOL_COUNT: int = 7  # foundation 4 reads + E4 +3 (class=api siblings: lookupproccode/diagcode/modcode)
# ----------------------------------------------------------------------------
...
def _maybe_install_test_client() -> None:
    ...
    When set to a module path like ``amd_codes_mcp.tests.fixtures.fake_amd_responses``,
    ...
    try:
        ...
        if hasattr(mod, "FakeAMDClient"):
            ...
            _LOG.info("amd-codes-mcp: using FakeAMDClient from %s", hook)
...
def build_server(*, settings: Settings | None = None, login: bool = True) -> Server:
    ...
    if login:
        ...
        if v.allow:
            try:
                client.login()
                _LOG.info("amd-codes-mcp: AMD login OK")
            except Exception as exc:  # noqa: BLE001
                _LOG.error("amd-codes-mcp: AMD login failed: %s", exc)
        else:
            _LOG.warning(
                "amd-codes-mcp: login rate-limited at boot "
    ...
    if visible_count != EXPECTED_TOOL_COUNT:
        raise RuntimeError(
            f"amd-codes-mcp: expected {EXPECTED_TOOL_COUNT} read-only "
            f"tools, got {visible_count}. Refusing to start."
        )
    _LOG.info("amd-codes-mcp: registered %d tools", visible_count)
...
def _install_shutdown_handlers(loop: asyncio.AbstractEventLoop) -> None:
    def _shutdown(sig: int) -> None:
        _LOG.info("amd-codes-mcp: received signal %d, shutting down", sig)
...
def main() -> None:
    ...
    if not os.environ.get("AMD_MCP_TEST_CLIENT"):
        try:
            ...
            print(f"amd-codes-mcp: {e}", file=sys.stderr)
            sys.exit(2)

    _LOG.info("amd-codes-mcp starting (allow_phi=%s)", s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-codes-mcp ready")
    ...
    try:
        ...
        loop.close()
        _LOG.info("amd-codes-mcp: exited cleanly")
...
def main_http() -> None:  # pragma: no cover
    ...
    port = int(os.environ.get("AMD_MCP_PORT", "8804"))
    _LOG.info("amd-codes-mcp-http starting on :%d (allow_phi=%s)", port, s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-codes-mcp-http ready")

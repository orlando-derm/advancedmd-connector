"""amd_ehr_get_ehr_immunizations — BETA EHR. Count only, no raw."""
from __future__ import annotations
from typing import Any
from amd_mcp_common.errors import safe_amd_call
from ._common import count_rows_for_tags, get_client, raw_to_dict

ACTION = "getehrimmunizations"
WRITE_ACTION = False
TIER = 2
PERMITTED_ACTIONS = ("getehrimmunizations",)
...
async def handle(*, patient_id: str) -> dict[str, Any]:
    ...
    raw_dict, err = safe_amd_call(client, action="getehrimmunizations", raw_to_dict_fn=raw_to_dict, **call_kwargs)
    if err is not None:
        return {"patient_id": patient_id, **err}
    return {
        "patient_id": patient_id,
        "count": count_rows_for_tags(raw_dict, "immunization", "vaccine"),
    }

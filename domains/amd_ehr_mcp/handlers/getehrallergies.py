"""amd_ehr_get_ehr_allergies — BETA EHR. Count only, no raw.

Aaron 2026-06-04: "it cannot do any math or aggregations properly."
EHR endpoints are Beta + PHI-heavy. Adam never enumerates them.
"""
...
ACTION = "getehrallergies"
WRITE_ACTION = False
TIER = 2
PERMITTED_ACTIONS = ("getehrallergies",)
...
async def handle(*, patient_id: str) -> dict[str, Any]:
    ...
    raw_dict, err = safe_amd_call(client, action="getehrallergies", raw_to_dict_fn=raw_to_dict, **call_kwargs)
    if err is not None:
        return {"patient_id": patient_id, **err}
    return {
        "patient_id": patient_id,
        "count": count_rows_for_tags(raw_dict, "allergy", "allergies"),
    }

"""amd-masterfiles-mcp MCP stdio server."""
...
_LOG = logging.getLogger("amd_masterfiles_mcp.server")


# ---- Structural safety belt -----------------------------------------------
WRITE_TOOLS_ENABLED: bool = False
DOMAIN: str = "masterfiles"
SERVER_NAME: str = "amd-masterfiles-mcp"
EXPECTED_TOOL_COUNT: int = 8
# ----------------------------------------------------------------------------
...
def _maybe_install_test_client() -> None:
    ...
    try:
        ...
        if hasattr(mod, "FakeAMDClient"):
            ...
            _LOG.info("amd-masterfiles-mcp: using FakeAMDClient from %s", hook)
...
def build_server(*, settings: Settings | None = None, login: bool = True) -> Server:
    ...
    if login:
        ...
        if v.allow:
            try:
                client.login()
                _LOG.info("amd-masterfiles-mcp: AMD login OK")
            except Exception as exc:
                _LOG.error("amd-masterfiles-mcp: AMD login failed: %s", exc)
        else:
            _LOG.warning(
                "amd-masterfiles-mcp: login rate-limited at boot "
    ...
    if visible_count != EXPECTED_TOOL_COUNT:
        raise RuntimeError(
            f"amd-masterfiles-mcp: expected {EXPECTED_TOOL_COUNT} read-only "
            f"tools, got {visible_count}. Refusing to start."
        )
    _LOG.info("amd-masterfiles-mcp: registered %d tools", visible_count)
...
def _install_shutdown_handlers(loop: asyncio.AbstractEventLoop) -> None:
    def _shutdown(sig: int) -> None:
        _LOG.info("amd-masterfiles-mcp: received signal %d, shutting down", sig)
...
def main() -> None:
    ...
    if not os.environ.get("AMD_MCP_TEST_CLIENT"):
        try:
            ...
            print(f"amd-masterfiles-mcp: {e}", file=sys.stderr)
            sys.exit(2)

    _LOG.info("amd-masterfiles-mcp starting (allow_phi=%s)", s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-masterfiles-mcp ready")
    ...
    try:
        ...
        loop.close()
        _LOG.info("amd-masterfiles-mcp: exited cleanly")
...
def main_http() -> None:  # pragma: no cover
    ...
    port = int(os.environ.get("AMD_MCP_PORT", "8807"))
    _LOG.info("amd-masterfiles-mcp-http starting on :%d (allow_phi=%s)", port, s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-masterfiles-mcp-http ready")

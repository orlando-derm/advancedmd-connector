...
C2.3: `_HANDLER_MODULES` populated with the three provider read
actions. Providers has no write stubs (no AMD-documented provider
write action — see C2.0 audit).
"""
...
from . import (
    getupdatedproviders,
    getupdatedreferringproviders,
    lookupprovider,
    # E3 additions per AMD_TOOLS_FULL_BUILDOUT_PLAN-final.txt:
    lookupprofile,
    lookuprefprovider,
)


# C2.3: three read handlers (no write stubs for providers).
# E3 (2026-06-04): +2 reads (lookupprofile, lookuprefprovider).
    ...
    *,
    ...
    for mod in _HANDLER_MODULES:
        ...
        if policy is None:
            ...
            import logging
            logging.getLogger("amd_providers_mcp.factory").warning(

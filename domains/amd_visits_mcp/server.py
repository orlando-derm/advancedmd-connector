"""amd-visits-mcp MCP stdio server.

Boot:

  1. Settings.from_env(domain="visits").
  ...
  5. Load per-action policies from knowledge/integrations/amd/visits/.
...
NOTE (C1.2 scaffold): No handlers or policies exist yet — those land
in C1.3 and C1.4. `EXPECTED_TOOL_COUNT = 0` until then; raising it is
part of the C1.3 wire-up. Until handlers exist, `build_server()` will
...
_LOG = logging.getLogger("amd_visits_mcp.server")


# ---- Structural safety belt -----------------------------------------------
WRITE_TOOLS_ENABLED: bool = False
DOMAIN: str = "visits"
SERVER_NAME: str = "amd-visits-mcp"
EXPECTED_TOOL_COUNT: int = 3  # 3 reads visible; addvisit write stub filtered + policies
# ----------------------------------------------------------------------------
...
def _maybe_install_test_client() -> None:
    ...
    When set to a module path like ``amd_visits_mcp.tests.fixtures.fake_amd_responses``,
    ...
    try:
        ...
        if hasattr(mod, "FakeAMDClient"):
            ...
            _LOG.info("amd-visits-mcp: using FakeAMDClient from %s", hook)
...
def build_server(*, settings: Settings | None = None, login: bool = True) -> Server:
    ...
    if login:
        ...
        if v.allow:
            try:
                client.login()
                _LOG.info("amd-visits-mcp: AMD login OK")
            except Exception as exc:  # noqa: BLE001
                _LOG.error("amd-visits-mcp: AMD login failed: %s", exc)
        else:
            _LOG.warning(
                "amd-visits-mcp: login rate-limited at boot "
    ...
    if visible_count != EXPECTED_TOOL_COUNT:
        raise RuntimeError(
            f"amd-visits-mcp: expected {EXPECTED_TOOL_COUNT} read-only "
            f"tools, got {visible_count}. Refusing to start."
        )
    _LOG.info("amd-visits-mcp: registered %d tools", visible_count)
...
def _install_shutdown_handlers(loop: asyncio.AbstractEventLoop) -> None:
    def _shutdown(sig: int) -> None:
        _LOG.info("amd-visits-mcp: received signal %d, shutting down", sig)
...
def main() -> None:
    ...
    if not os.environ.get("AMD_MCP_TEST_CLIENT"):
        try:
            ...
            print(f"amd-visits-mcp: {e}", file=sys.stderr)
            sys.exit(2)

    _LOG.info("amd-visits-mcp starting (allow_phi=%s)", s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-visits-mcp ready")
    ...
    try:
        ...
        loop.close()
        _LOG.info("amd-visits-mcp: exited cleanly")
...
def main_http() -> None:  # pragma: no cover
    ...
    port = int(os.environ.get("AMD_MCP_PORT", "8802"))
    _LOG.info("amd-visits-mcp-http starting on :%d (allow_phi=%s)", port, s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-visits-mcp-http ready")

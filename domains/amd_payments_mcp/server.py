"""amd-payments-mcp MCP stdio server.

Boot:

  1. Settings.from_env(domain="payments").
  ...
  5. Load per-action policies from knowledge/integrations/amd/payments/.
...
C5.2 scaffold: handlers / policies in C5.3-C5.4. After C5.3 the
EXPECTED_TOOL_COUNT is 1 (`gettxhistory` read; `addpayments` write
stub filtered out).
"""
...
_LOG = logging.getLogger("amd_payments_mcp.server")


# ---- Structural safety belt -----------------------------------------------
WRITE_TOOLS_ENABLED: bool = False
DOMAIN: str = "payments"
SERVER_NAME: str = "amd-payments-mcp"
EXPECTED_TOOL_COUNT: int = 1  # 1 read visible (gettxhistory); addpayments write stub filtered
# ----------------------------------------------------------------------------
...
def _maybe_install_test_client() -> None:
    ...
    When set to a module path like ``amd_payments_mcp.tests.fixtures.fake_amd_responses``,
    ...
    try:
        ...
        if hasattr(mod, "FakeAMDClient"):
            ...
            _LOG.info("amd-payments-mcp: using FakeAMDClient from %s", hook)
...
def build_server(*, settings: Settings | None = None, login: bool = True) -> Server:
    ...
    if login:
        ...
        if v.allow:
            try:
                client.login()
                _LOG.info("amd-payments-mcp: AMD login OK")
            except Exception as exc:  # noqa: BLE001
                _LOG.error("amd-payments-mcp: AMD login failed: %s", exc)
        else:
            _LOG.warning(
                "amd-payments-mcp: login rate-limited at boot "
    ...
    if visible_count != EXPECTED_TOOL_COUNT:
        raise RuntimeError(
            f"amd-payments-mcp: expected {EXPECTED_TOOL_COUNT} read-only "
            f"tools, got {visible_count}. Refusing to start."
        )
    _LOG.info("amd-payments-mcp: registered %d tools", visible_count)
...
def _install_shutdown_handlers(loop: asyncio.AbstractEventLoop) -> None:
    def _shutdown(sig: int) -> None:
        _LOG.info("amd-payments-mcp: received signal %d, shutting down", sig)
...
def main() -> None:
    ...
    if not os.environ.get("AMD_MCP_TEST_CLIENT"):
        try:
            ...
            print(f"amd-payments-mcp: {e}", file=sys.stderr)
            sys.exit(2)

    _LOG.info("amd-payments-mcp starting (allow_phi=%s)", s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-payments-mcp ready")
    ...
    try:
        ...
        loop.close()
        _LOG.info("amd-payments-mcp: exited cleanly")
...
def main_http() -> None:  # pragma: no cover
    ...
    port = int(os.environ.get("AMD_MCP_PORT", "8806"))
    _LOG.info("amd-payments-mcp-http starting on :%d (allow_phi=%s)", port, s.allow_phi)
    server = build_server(settings=s, login=True)
    _LOG.info("amd-payments-mcp-http ready")
